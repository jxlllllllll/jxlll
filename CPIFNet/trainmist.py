...
inputPathTrain="/home/liu/jxl/misttrain_16_256/test/"
targetPathTrain="/home/liu/jxl/densetrain_16_256/hr_256/"
#
inputPathTest="/home/liu/jxl/misttrain_16_256/test/"
targetPathTest="/home/liu/jxl/densetrain_16_256/hr_256/"

net=IENet()
net=net.cuda()
criterion1=nn.L1Loss()
criterion2=color_loss()
criterion3=VGG_loss().cuda()
criterion4=MS_SSIM()
...
valueLoader=DataLoader(dataset=datasetValue,batch_size=1)
resultPathTest = 'F:\\Py1\\SWsr\\result\\'
resultPathTest2 = 'F:\\Py1\\SWsr\\result2\\'
datasetTest=TestDataSet(inputPathTest)
epochLoss_best=10000000
accumulation_steps = 4
testLoader=DataLoader(dataset=datasetTest,batch_size=1)
print("--------------------开始训练----------------------\n")
if os.path.exists('./canshu_1_1_1_2_new.pth'):
    net.load_state_dict(torch.load('./canshu_1_1_1_2_new.pth'))
for epoch in range(Epoch):
    ...
    for index,(x,y) in enumerate(iters,0):
        net.zero_grad()
        #save_image(x, resultPathTest + str(index + 1).zfill(3) + ".png")
        #save_image(y, resultPathTest2 + str(index + 1).zfill(3) + ".png")
        optimizer.zero_grad()
        ...
        # print(criterion4(output_train,target))
        loss=criterion1(output_train,target)+criterion2(output_train,target)+criterion3(output_train,target)+2*(1-criterion4(output_train,target))
        loss.backward()
        optimizer.step()
        epochLoss+=loss.item()
    ...
    for index, (x, y) in enumerate(valueLoader, 0):
        ...
        with torch.no_grad():
            ...
            loss = criterion1(output_value, target_value)
    ...
    if epochLoss_test < epochLoss_best:
        ...
        best_epoch = epoch
        torch.save(net.state_dict(), './canshu_1_1_1_2_new.pth')
        loss_list.append(epochLoss)  # 插入每次训练的损失值
        torch.save(net.state_dict(), './canshu_1_1_1_2_new.pth')  # 每次训练结束保存模型参数
