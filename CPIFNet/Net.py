...
class resnet1(nn.Module):
    ...
    def forward(self,x):
        ...
        y2=self.relu(self.conv2(x1))
        x2=torch.cat((x1,y2),dim=1)
        y3=self.relu(self.conv3(x2))
        x3=torch.cat((x2,y3),dim=1)
        y4=self.conv4(x3)
        y=x+y4
        return y
class resnet2(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=48, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv2d(in_channels=80, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.relu=nn.ReLU()
    def forward(self,x):
        y1=self.relu(self.conv1(x))
        #print(y1.shape)
...
class IENet(nn.Module):
    def __init__(self):
        ...
        self.relu=nn.ReLU()
        self.res=resnet1()
        self.conv2=nn.Conv2d(in_channels=32,out_channels=3,kernel_size=3,padding=1,stride=1)
        self.sig=nn.Sigmoid()
        self.res2=resnet2()
    def forward(self, x):
        b0=self.relu(self.conv1(x))
        r1=self.res(b0)
        r2=self.res(r1)
        r3=self.res(r2)
        r4=self.res2(torch.cat((r3,r2),dim=1))
        r5=self.res2(torch.cat((r4,r1),dim=1))
        b1=self.conv2(torch.cat((r5,b0),dim=1))
        b2=self.sig(b1)
        return b2
class IFNet(nn.Module):
    def __init__(self):
        ...
        self.a1= nn.Sequential(*[
            nn.Conv2d(in_channels=48,out_channels=16,kernel_size=3,padding=1,stride=1),
            nn.ReLU()   
    ...
    def forward(self,x1,x2):
        x1_1=self.conv1(x1)
        x1_1=self.relu(x1_1)
        x1_2=self.first(x1_1)
        x1_3=self.first(x1_2)
        x1_4=self.first(x1_3)
        x2_1=self.conv1(x2)
        x2_1=self.relu(x2_1)
        x2_1=self.first(x2_1)
        x2_2=self.first(x2_1)
        x2_3=self.first(x2_2)
        x2_4=self.first(x2_3)
        y1=torch.cat([x1_1,x2_1],1)
        y2=torch.cat([x1_2,x2_2],1)
        y3=torch.cat([x1_3,x2_3],1)
        y4=torch.cat([x1_4,x2_4],1)

        out1=self.second(y4)
        out2=torch.cat([y4,out1],1)
        out3=self.a1(out2)

        out4=torch.cat([y3,out3],1)
        out5=self.a1(out4)

        out6=torch.cat([y2,out5],1)
        out7=self.a1(out6)

        out8=torch.cat([y1,out7],1)
        y=self.a2(out6)
